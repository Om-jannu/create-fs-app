#!/bin/bash

# Bump version script
# Usage: ./scripts/bump-version.sh [patch|minor|major]

set -e

# Colors
RED='\033[0;31m'
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
BLUE='\033[0;34m'
NC='\033[0m' # No Color

# Get release type
RELEASE_TYPE=${1:-patch}

if [[ ! "$RELEASE_TYPE" =~ ^(patch|minor|major)$ ]]; then
  echo -e "${RED}Error: Invalid release type. Use: patch, minor, or major${NC}"
  exit 1
fi

echo -e "${BLUE}📝 Bumping version ($RELEASE_TYPE)...${NC}\n"

# Get current version
CURRENT_VERSION=$(node -p "require('./package.json').version")
echo -e "${BLUE}Current version: ${CURRENT_VERSION}${NC}"

# Bump version in package.json
npm version $RELEASE_TYPE --no-git-tag-version

# Get new version
NEW_VERSION=$(node -p "require('./package.json').version")
echo -e "${GREEN}New version: ${NEW_VERSION}${NC}"

echo -e "${GREEN}✅ Version updated in package.json${NC}"
echo -e "${GREEN}   (src/index.ts reads version from package.json dynamically)${NC}"

echo -e "\n${YELLOW}Next steps:${NC}"
echo -e "1. Update CHANGELOG.md with changes"
echo -e "2. Commit: ${BLUE}git add . && git commit -m 'Bump version to ${NEW_VERSION}'${NC}"
echo -e "3. Push: ${BLUE}git push${NC} (merge PR to master/main if applicable — this will NOT publish to npm)"
echo -e "4. Tag and push the tag to publish: ${BLUE}git tag v${NEW_VERSION} && git push origin v${NEW_VERSION}${NC}"
echo -e "5. Pushing the tag triggers GitHub Actions to publish v${NEW_VERSION} to npm and create a GitHub Release"
