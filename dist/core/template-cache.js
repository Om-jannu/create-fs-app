 ...
 */
export async function isTemplateCached(template) {
 ...
 */
export async function getCachedTemplatePath(template) {
 ...
 * Cache a template
 */
export async function cacheTemplate(template, branch = 'main') {
    await ensureCacheDir();
    const cacheKey = getCacheKey(template);
    const cachePath = path.join(CACHE_DIR, cacheKey);
 ...
 */
export async function copyCachedTemplate(cachePath, targetDir) {
