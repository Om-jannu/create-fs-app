 ...
 */
export function displayAvailableTemplates() {
    const templates = listAllTemplates();
    if (templates.length === 0) {
        const noTemplatesMsg = boxen(chalk.yellow.bold('⚠️  No templates available yet\n\n') +
    ...
    const usageBox = boxen(chalk.white.bold('Usage:\n\n') +
...
export function getTemplateByName(name) {
    const templates = listAllTemplates();
    return templates.find(t => t.key === name || t.key.includes(name));
}
/**
...
export function searchTemplates(keyword) {
    ...
    return templates.filter(({ key, metadata }) => key.includes(lowerKeyword) ||
 ...
 */
export function displayTemplateStats() {
