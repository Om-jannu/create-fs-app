 ...
 */
export async function runHealthCheck(projectDir = process.cwd()) {
    const checks = [];
 ...
 */
export function displayHealthCheckResults(result) {
    console.log();
...
async function checkPackageJson(projectDir) {
    try {
...
async function checkGitRepo(projectDir) {
    try {
...
async function checkTypeScriptConfig(projectDir) {
    try {
...
async function checkMonorepoStructure(projectDir) {
    try {
...
async function checkBuildScripts(projectDir) {
    try {
        const pkgPath = path.join(projectDir, 'package.json');
        const content = await fs.readFile(pkgPath, 'utf-8');
        const pkg = JSON.parse(content);
